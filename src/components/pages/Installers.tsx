...
const Installers: React.FC = () => {
  ...
  const installersWithRatings = dataset.installers.filter(i => i.rating !== null).length;
  const batteryQualified = dataset.installers.filter(i => i.tags.some(t => t.toLowerCase().includes('battery'))).length;

  const filteredInstallers = React.useMemo(() => {
    return dataset.installers.filter((installer) => {
      const matchesSearch = 
        installer.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        installer.certificationBody.toLowerCase().includes(searchTerm.toLowerCase()) ||
        installer.tags.some(tag => tag.toLowerCase().includes(searchTerm.toLowerCase()));
      
      const matchesLocation = 
        installer.address.toLowerCase().includes(locationTerm.toLowerCase()) ||
        installer.cities.some(city => city.toLowerCase().includes(locationTerm.toLowerCase()));

      return matchesSearch && matchesLocation;
    });
  }, [searchTerm, locationTerm]);

  const visibleInstallers = filteredInstallers.slice(0, 100);

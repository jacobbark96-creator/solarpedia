...
// Read existing terms
// We'll just rewrite the whole file with the old + new terms to be safe.
...
const newTerms = [
  { term: 'AC Coupled Battery', definition: 'A battery system that connects to the alternating current (AC) side of your home wiring, requiring its own separate inverter. Often used when retrofitting batteries to existing solar arrays.' },
  { term: 'DC Coupled Battery', definition: 'A battery system that connects directly to the direct current (DC) side of your solar panels, sharing a single hybrid inverter. Generally more efficient than AC coupled systems.' },
  { term: 'Agrivoltaics', definition: 'The co-development of land for both solar photovoltaic power and agriculture, such as raising crops or grazing sheep beneath raised solar panels.' },
  { term: 'Albedo', definition: 'The measure of how much sunlight is reflected by a surface. High albedo surfaces (like snow or white roofs) reflect more light, which can boost the output of bifacial solar panels.' },
  { term: 'Base Load', definition: 'The minimum level of electricity demand required continuously over a 24-hour period to keep essential appliances running (e.g., fridge, router, standby devices).' },
  { term: 'Bifacial Panels', definition: 'Solar panels designed with solar cells on both the front and back, allowing them to capture direct sunlight from above and reflected sunlight from below. Mostly used in ground-mounted or commercial setups.' },
  { term: 'Blackout Protection (EPS)', definition: 'An Emergency Power Supply (EPS) feature that allows a solar battery system to continue powering essential circuits in the house during a grid power cut.' },
  { term: 'Brownout', definition: 'A temporary drop in electrical voltage from the grid, which can cause lights to dim. Advanced solar inverters can protect home appliances from brownouts.' },
  { term: 'Cell', definition: 'The basic building block of a solar panel. Most residential solar panels consist of 54, 60, or 72 individual solar cells wired together.' },
  { term: 'Conduit', definition: 'The protective tubing (usually PVC or metal) used to safely route electrical wiring from the solar panels on the roof down to the inverter.' },
  { term: 'Current', definition: 'The flow of electrical charge through a conductor, measured in Amperes (Amps).' },
  { term: 'Curtailment', definition: 'When a solar inverter is forced to reduce its power output below what the panels are capable of generating, usually due to grid voltage limits or export caps set by the DNO.' },
  { term: 'Cycle Life', definition: 'The number of complete charge and discharge cycles a battery can perform before its capacity degrades to a specified percentage (usually 80% or 70% of its original capacity).' },
  { term: 'Disconnect Switch (Isolator)', definition: 'A safety switch used to manually cut off the flow of electricity between the solar panels, the inverter, or the grid. Required by UK electrical regulations.' },
  { term: 'Dynamic Pricing', definition: 'Electricity tariffs where the price changes frequently (sometimes every 30 minutes) based on wholesale energy costs. Ideal for homes with smart batteries.' },
  { term: 'Energy Density', definition: 'The amount of energy stored in a given system or region of space per unit volume. NMC batteries have higher energy density than LFP, making them smaller and lighter.' },
  { term: 'Export Limitation', definition: 'A system setup that actively monitors home energy use and restricts the amount of solar power sent to the grid to comply with strict DNO limits.' },
  { term: 'Grid Edge', definition: 'The point where the local electricity distribution network connects to a consumer’s property (e.g., the smart meter).' },
  { term: 'Hertz (Hz)', definition: 'The unit of frequency. In the UK, the standard AC electrical grid operates at a frequency of 50 Hz.' },
  { term: 'Interconnection', definition: 'The physical and legal process of connecting a home solar PV system to the National Grid.' },
  { term: 'Inverter Efficiency', definition: 'The ratio of usable AC output power to DC input power. High-quality string inverters typically operate at 97% to 98% efficiency.' },
  { term: 'Islanding', definition: 'A dangerous condition where a grid-tied solar system continues to supply power to the local grid during a wider power outage.' },
  { term: 'Junction Box', definition: 'An enclosure attached to the back of a solar panel that houses the electrical connections and bypass diodes.' },
  { term: 'Lifecycle Assessment (LCA)', definition: 'The evaluation of the environmental impacts associated with all stages of a solar panel’s life, from raw material extraction through materials processing, manufacture, distribution, use, and recycling.' },
  { term: 'Maximum Power Point (MPP)', definition: 'The exact point on a solar panel\'s voltage-current curve where it produces the absolute maximum electrical power.' },
  { term: 'N-Type Silicon', definition: 'A newer, more advanced type of silicon wafer manufacturing that treats the silicon with phosphorus. N-type panels degrade slower and perform better in high heat than older P-type panels.' },
  { term: 'Nominal Capacity', definition: 'The total theoretical energy capacity of a battery. The "usable capacity" is usually lower to protect the battery from degrading.' },
  { term: 'Optimizer', definition: 'A small device attached to individual solar panels that performs MPPT at the panel level, mitigating the effects of shade without converting the DC power to AC on the roof.' },
  { term: 'Orientation', definition: 'The compass direction that the solar panels face (Azimuth).' },
  { term: 'P-Type Silicon', definition: 'The traditional method of manufacturing silicon wafers by treating them with boron. They are cheaper to produce but degrade slightly faster than N-type panels.' },
  { term: 'Peak Demand', definition: 'The time of day when electricity consumption on the National Grid is at its highest, typically between 4pm and 7pm in the UK.' },
  { term: 'Phantom Load', definition: 'The electricity consumed by appliances and electronics while they are switched off or in standby mode.' },
  { term: 'Pitch', definition: 'The angle or steepness of a roof. In the UK, a pitch between 30° and 40° is considered optimal for solar generation.' },
  { term: 'Photovoltaic Effect', definition: 'The physical and chemical phenomenon where light (photons) strikes a semiconductor material (like silicon) and knocks electrons loose, creating an electrical current.' },
  { term: 'Power Conditioner', definition: 'A device intended to improve the quality of the power that is delivered to electrical load equipment.' },
  { term: 'Radiant Energy', definition: 'Energy transmitted in wave motion, especially electromagnetic wave motion (like sunlight).' },
  { term: 'Self-Consumption', definition: 'The percentage of the solar energy generated by your system that is actually used inside your home, rather than being exported to the grid.' },
  { term: 'Smart Grid', definition: 'An electricity supply network that uses digital communications technology to detect and react to local changes in usage.' },
  { term: 'Solar Fraction', definition: 'The percentage of a building’s total energy demand that is supplied by solar power.' },
  { term: 'Solar Window', definition: 'The period of the day when the sun is unshaded and high enough in the sky to produce significant solar power.' },
  { term: 'State of Charge (SoC)', definition: 'The current level of charge in a battery, expressed as a percentage of its total capacity (e.g., your phone battery is at 80% SoC).' },
  { term: 'Surge Capacity', definition: 'The maximum amount of short-term power an inverter or battery can provide to start up heavy appliances (like a heat pump or air conditioner) that require a sudden spike in electricity.' },
  { term: 'Temperature Coefficient', definition: 'A rating that shows how much a solar panel\'s efficiency drops for every 1°C increase in temperature above 25°C. Lower is better, as panels lose efficiency when they get hot.' },
  { term: 'Tracking Array', definition: 'A solar panel mounting system that physically moves throughout the day to follow the sun’s path, maximizing generation. Very rare in UK domestic settings.' },
  { term: 'Utility-Scale Solar', definition: 'Massive solar farms that generate electricity directly for the National Grid, rather than for a specific building or home.' },
  { term: 'Voltage (V)', definition: 'The measure of electrical potential difference between two points. It is the "pressure" that pushes electrons through a wire.' },
  { term: 'Watt-hour (Wh)', definition: 'A unit of energy equivalent to one Watt of power expended for one hour.' },
  { term: 'Yield Curve', definition: 'A graph showing the energy production of a solar system over a specific period, usually rising in the morning, peaking at midday, and falling in the evening.' }
];

const allTerms = [...oldTerms, ...newTerms];
...
export const glossaryData: GlossaryTerm[] = ${JSON.stringify(allTerms, null, 2)};
`;

fs.writeFileSync(path.join(__dirname, '../src/data/glossary.ts'), fileContent);
console.log('Glossary data expanded to ' + allTerms.length + ' terms.');